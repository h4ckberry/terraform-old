...
resource "azurerm_public_ip" "mke_lb_pub_ip" {
  ...
  domain_name_label = format("mke-hackberry")
...
resource "azurerm_dev_test_global_vm_shutdown_schedule" "schedule_shutdown_mke_master" {
  count              = var.master_count
  virtual_machine_id = azurerm_virtual_machine.mke_master[0].id
  location           = var.location
  enabled            = true
